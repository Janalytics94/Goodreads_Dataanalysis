...
abstract: |
 This termpaper covers an descriptive and exploratory analysis of the goodreads data set, which includes a wide variety of books and their corresponding information.  In the following, the data will be visualized to  illustrate the content and explore possible dependecies and correlations of the features. Finally a K-Means cluster analysis with respect to the price and the number of pages will be performed in order to find out, if there are any patterns or peculiarities between those two features.
link-citations: true
linkReferences: true
geometry: margin=1in
fontfamily: mathpazo
...
## Meta Information
The `Goodreads-books` data set is available on the platform [www.kaggle.com](`www.kaggle.com`) and was created on May 25 in year 2019 using the API [^1]
...
```
Table \ref{tab:tab1} sums up the features which are considered in this analysis.
...
Table \ref{tab:tab2} gives an overview of the data set.
...
```
Our data set contains 11.127 different books and 13 columns, which represent the features of each book. Table \ref{tab:tab3} summarizes the data types of each feature.
...
The following Table \ref{tab:tab4} shows that only the feature `price` contains 1.898 missing values.
```{r tab4}
...
\
The following questions shall guide us through our data exploration.
\
...
\
According to Figure \ref{fig:fig1}, the most rated books are those, which are part of a series of books, such as Twillight or Harry Potter. 
```{r fig1, fig.align="center", fig.height = 3, fig.width = 6, fig.cap="The most rated Books", fig.pos="H"}
#Most rated
most_rated <- arrange(goodreads, desc(ratings_count))
fig1 <- ggplot(head(most_rated,10), aes(x=title, y=ratings_count, fill=title), size=1.3)+
...
The Harry Potter Saga appears several times in our plot. We can assume that they belong to the most read books as all were rated pretty often. The book Twillight has received with 4.597.666 by far the most ratings.
...
The Book Twillight and the third book of the Harry Potter Series appear again in the Bar plot in Figure \ref{fig:most_text_reviews_plot}. Also the book The Catcher in the Rye appears in both plots, which lets us assume that there might be a connection between the variables `text_reviews_count` and `ratings_count`.
...
```
\
According to the Figure \ref{fig:most_publ_books_plot} the authors Stephen King and P.G. Wodehouse are, with a number of 40 books published, the front-runners, followed by Rumiko Takahsashi with 39 published books.
...
The Table \ref{tab:moments_price_tab} illustrates that the minimum price of a book is \$ 0.01 and the maximum price is at \$ 249.914,95. 
Looking at the following Box plot in Figure \ref{fig:box_price}  we see that only one book has such a high price. 

```{r box_price, fig.cap="Box Plot Price in USD with Outliers", fig.height = 3, fig.width = 6, fig.margin=T, fig.align="center", fig.pos="H"}
...
As we can see in our Scatter plot in Figure \ref{fig:price_scatter} we separated the books in fife different prices classes. The range of the different classes are $D\in (0, 10]$ for the first, $D\in (10, 50]$ for the second, $D\in (50, 100]$ for the third, $D\in (100, 200]$ for the forth and $D\in (200, 800]$ for the fifth price class.
...
If we look at the Density plot in Figure \ref{fig:dens_p} of our price data we see a right-skewed or positive-skewed distribution.
...
```{r genre, fig.align="center", fig.height =3, fig.width = 6, fig.margin=T, fig.cap="Representation of Genre of Books", fig.pos="H"}
# Genre data 
...
As we can see in Table \ref{tab:Stat_Moments_average_rating} and in the Box plot in Figure \ref{fig:bp_avr}, the evaluation of the book is between $D\in [0.5]$, the median is 3.96 and the mean is 3.93. 
...
```{r bp_avr, fig.align="center", fig.height = 4, fig.width = 4, fig.margin=T, fig.cap="Box Plot of Average Rating", fig.pos="H"}
#Boxplot
...
The Density plot in Figure \ref{fig:den} shows a left-skewed distribution and the `average_rating` might be normally distributed. The majority of the books are rated between 3.7 and 4.3 approximately. Books having scores near 5 are extremely rare. Also we want to find out which values received the value 0. This leads also to the conclusion that if a person does read a book and end up not liking it, it still get a minimum of around 2 stars, just for the effort they put up to read the book.
...
\
We plot the five books with the most pages which is illustrated in the Bar plot in Figure \ref{fig:most_pages_plot}. The books which represent our most significant outliers are The Complete Aubrey/Maturin Novels (5 Volumes) and The Second World War. The other books which are between a size of pages between 3.000 and 40.000 are the Summa Theologica 5 Vols., Remembrance of Things Past (Boxed Set) and the Harry Potter Collection. The data points which appear in this plot are mostly listed as sets, which would explain the high number of pages.
\
...
Furthermore there are books which have a considerably low number of pages. Those are listed in the following Table \ref{tab:Audiobooks} and are probably audio books.
```{r Audiobooks}
...
The following Figure \ref{fig:plot_np_avg} of multiple Box plots illustrates a slight decrease in the average rating with the increase of the number of pages from 0 to 300. Then there is a reverse trend.
...
Looking more closer into the variable `num_pages` we want to find out how the number of pages is distributed. The  Density plot in Figure \ref{fig:dist_numP} shows also as right skewed distribution as the right tail is significantly drawn out compared to the other side.  
...
As we can see in the following Bar plot in Figure \ref{fig:l} most books are in English which is split into different sub groups of American, British, Canadian and Other English.
...
```{r figCorr, fig.height = 2, fig.width = 4, fig.margin=T, fig.cap="Correlation between Numeric Variables", fig.pos="H"}
# Plot
figCorr<-ggcorrplot(corr, hc.order=T, outline.color = "white",
           ggtheme=theme_bw(),
...
## Are there any Patterns or Peculiarities between the Price of the Books and the Number of Pages?

At the end of our data analysis we want to explore, if we could find any patterns or peculiarities between the number of pages of books and the price. We will use the K-Means Clustering which is a simple unsupervised machine learning algorithm that groups a data set into user-specified number (k) of clusters.
The Scatter plot regarding Figure \ref{fig:price_num_pa_scatter} could lead to the assumption shows a slight correlation between those two variables.
...
The plot in Figure \ref{fig:optimal_clusters_plot} looks like the bend of an elbow. Usually you chose the number of clusters where the line bends but in our case it is not clear which number ob clusters we should take so we will perform a cluster analysis using $k = 5 = 6$ clusters.
...
Looking at both Cluster plots in Figure \ref{fig:clusterplot5} and Figure \ref{fig:clusterplot6} we can see that there is not a significant difference between selecting $k=5$ or $k=6$ clusters. 
